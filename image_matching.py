...
def eliminate_dup(test, indices):
    ids = [str(index_to_id(idx, df)) for idx in indices]
    myID = '.'.join(test.split(".")[:-1])
    if myID in ids:
        indices.remove([ids.index(myID)])
        indices.append(np.argsort(dist)[count])
    return new_indices
...
def get_similar_art(extracted_features, new_art_ef, test, feature_df, df, count=5,distance = "euclidean"):
    if distance == "euclidean":
        ...
        indices = eliminate_dup(test, indices)
